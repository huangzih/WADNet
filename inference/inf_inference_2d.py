...
with open('./data/task1-2d.csv', 'w') as f:
    f.write('pos;length\n')
    for word in d2_data:
        f.write(word+';')
...
data = pd.read_csv('./data/task1-2d.csv', sep=';')
    ...
    def __getitem__(self, index): 
        ...
        if fix_length == ori_length:
            ...
            for i in [0, ori_length-fix_length]: 
                ...
                data_seq_list.append(torch.stack((seq_x, seq_y), dim = 0))
...
model = AnDiModel(2, 64, 3, 1).to(device)
...
output_list_final = (output_list*1.0129).clip(0.05,2.0)

with open('./output/task1-2d.txt', 'w') as f:
