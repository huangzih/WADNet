...
with open('./data/task2-3d.csv', 'w') as f:
    f.write('pos;length\n')
    for word in d3_data:
        f.write(word+';')
...
data = pd.read_csv('./data/task2-3d.csv', sep=';')
    ...
    def __getitem__(self, index): 
        ...
        if fix_length == ori_length:
            ...
            for i in [0, ori_length-fix_length]:
                ...
                seq = torch.stack((seq_x, seq_y, seq_z), dim = 0)
...
model = AnDiModel(3, 64, 3, 5).to(device)
...
with open('./output/task2-3d.txt', 'w') as f:
