...
with open('./data/task2-1d.csv', 'w') as f:
    f.write('pos;length\n')
    for word in d1_data:
        f.write(word+';')
...
data = pd.read_csv('./data/task2-1d.csv', sep=';')
...
class AnDiDataset(Dataset):
    ...
    def __getitem__(self, index): 
        ...
        if fix_length == ori_length:
            return data_seq.unsqueeze(-1).permute(1,0), fix_length, 1
        else:
            data_seq_list = []
            for i in [0, ori_length-fix_length]: 
                seq = data_seq[i:i+fix_length].unsqueeze(-1).permute(1,0)
...
with open('./output/task2-1d.txt', 'w') as f:
